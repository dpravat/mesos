    ...
    const list<Option<ContainerLaunchInfo>>& launchInfos)
{
  ...
  return logger->prepare(executorInfo, directory)
    ...
    Try<Nothing> pipes = os::pipe(pipe.data());
    if (pipes.isError())
    {
      std::cerr << "Failed to create pipe " << pipes.error() << std::endl;
      abort();
    }
...
#ifndef __WINDOWS__
    launchFlags.pipe_read = pipe[0];
    launchFlags.pipe_write = pipe[1];
#else
    // NOTE: On windows we need to pass `Handle`s between processes, as fds
    // are not unique across processes.
    launchFlags.pipe_read = os::fd_to_handle(pipe[0]);
    launchFlags.pipe_write = os::fd_to_handle(pipe[1]);
#endif // __WINDOWS
    ...
    return isolate(containerId, pid)
      .then(defer(self(),
                  ...
                  user,
                  slaveId))
      .then(defer(self(), &Self::exec, containerId, pipe[1]))
      .onAny([pipe]() { os::close(pipe[0]); })
      .onAny([pipe]() { os::close(pipe[1]); });
  }));
}
