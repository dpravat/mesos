    ...
    Option<string>& rootfs)
{
  ...
  Try<Nothing> cloexec = os::cloexec(pipe[0]);
  if (cloexec.isError()) {
    cerr << "Failed to cloexec(pipe[0]): " << cloexec.error() << endl;
    abort();
  }

  cloexec = os::cloexec(pipe[1]);
  if (cloexec.isError()) {
  ...
  if (pid == 0) {
    ...
    if (write(pipe[1], &pid, sizeof(pid)) != sizeof(pid)) {
  ...
  if (read(pipe[0], &pid, sizeof(pid)) == -1) {
