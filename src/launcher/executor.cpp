...
#ifndef __WINDOWS__
    pid = launchTaskPosix(task.get(), command, argv, override, rootfs);
#else
    // Windows processe is started using `CREATE_SUSPENDED` flag and is part
    // of a job object. While the process handle is kept open the reap function
    // will work.
    PROCESS_INFORMATION pi =
        launchTaskWindows(task.get(), command, argv, override, rootfs);
    pid = pi.dwProcessId;
