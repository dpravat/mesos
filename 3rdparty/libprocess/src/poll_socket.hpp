...
{
public:
  static Try<std::shared_ptr<Socket::Impl>> create(const FileDesc& s);

  PollSocketImpl(const FileDesc& s) : Socket::Impl(s) {}
  ...
  virtual Future<size_t> sendfile(const FileDesc& fd, off_t offset, size_t size);
