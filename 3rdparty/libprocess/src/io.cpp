...
Future<Nothing> splice(const FileDesc& from, const FileDesc& to, size_t chunk)
{
...
Future<string> read(const FileDesc& fd)
{
  process::initialize();
...
Future<Nothing> write(const FileDesc& fd, const string& data)
{
  process::initialize();
...
Future<Nothing> redirect(const FileDesc& from, Option<FileDesc> to, size_t chunk)
{
  ...
  if (to.isNone()) {
    ...
    Try<FileDesc> open = os::open("/dev/null", O_WRONLY | O_CLOEXEC);
...
Future<string> peek(const FileDesc& fd, size_t limit)
{
  process::initialize();
