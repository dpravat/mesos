...
Try<std::shared_ptr<SocketImpl>> PollSocketImpl::create(const int_fd& s)
{
...
Future<int_fd> accept(const int_fd& fd)
{
...
Future<std::shared_ptr<SocketImpl>> PollSocketImpl::accept()
{
  return io::poll(get(), io::READ)
    ...
    .then([](const int_fd& s) -> Future<std::shared_ptr<SocketImpl>> {
...
Future<size_t> PollSocketImpl::sendfile(const int_fd& fd, off_t offset,
                                        size_t size) {
  return io::poll(get(), io::WRITE)
    .then(lambda::bind(
        &internal::socket_send_file,
