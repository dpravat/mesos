...
#ifdef __WINDOWS__
    [process]() { os::close(process.data->processInformation.hProcess); };
#else
    []() {};
#endif // __WINDOWS__
  process::reap(process.data->pid)
    .onAny(
        lambda::bind(internal::cleanup, lambda::_1, promise, process,
                     cleanup_hook));

#ifdef __WINDOWS__
  ::CloseHandle(process.data->processInformation.hThread);
#endif // __WINDOWS__
