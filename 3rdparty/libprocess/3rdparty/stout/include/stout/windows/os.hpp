...
//         implementation of `long` on the vast majority of produciton servers
...
inline Result<pid_t> waitpid(long pid, int* status, int options)
{
  ...
  if (pid <= 0) {
    errno = ENOSYS;
    return ErrnoError(
        "os::waitpid: Value of pid is '" + std::to_string(pid) +
        "'; the Windows implementation currently does not allow values <= 0");
  } else if (options != 0 && options != WNOHANG) {
    // In plain English: we only support `options == 0` or `options ==
    // WNOHANG`. That is, no other flags are supported.
    errno = ENOSYS;
    return ErrnoError(
        "os::waitpid: Only flag `WNOHANG` is implemented on Windows");
  }

  // TODO(hausdorff): check pid is one of the child processes if not, set
  // `errno` to `ECHILD` and return -1.

  // Open the child process as a safe `SharedHandle`.
  const HANDLE process = ::OpenProcess(PROCESS_QUERY_INFORMATION | SYNCHRONIZE,
  ...
  if (options == 0 && !state_signaled) {
    ...
    errno = ECHILD;
    return WindowsError(
        "os::waitpid: Failed to wait for pid '" + std::to_string(pid) +
        "'. `::WaitForSingleObject` should have waited for child process to " +
        "exit, but returned code '" + std::to_string(wait_results) +
        "' instead");
    ...
    return WindowsError(
        ...
        "but instead returned code '" + std::to_string(wait_results) + "'");
  }

  if (!wait_for_child && wait_results == WAIT_TIMEOUT) {
    // Success. `ENOHANG` was set and we got a timeout, so return 0.
    return None();
  }
