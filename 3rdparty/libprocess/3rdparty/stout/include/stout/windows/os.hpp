...
// `create_job` function create a job object whose name is derived
// from the `pid` and associates the  process with the job object.
// Every process started by the process which is part of the job
...
// is associated with a job object whose name is derive from it.
// Every process started by the process which is part of the job
