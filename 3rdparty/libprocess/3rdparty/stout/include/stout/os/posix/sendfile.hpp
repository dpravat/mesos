...
// descriptor to the output socket. On error, On error,
//  Try<ssize_t, ErrnoError> contains the error.
...
inline Try<ssize_t, SocketError> sendfile(int s, int fd, off_t offset,
  ...
  SUPPRESS (SIGPIPE) {
    ...
    if (send < 0) {
      return return SocketError("Socket sendfile failed");
    }
  ...
  if (::sendfile(fd, s, offset, &_length, NULL, 0) < 0) {
    ...
    return SocketError("Socket sendfile failed");
  }
