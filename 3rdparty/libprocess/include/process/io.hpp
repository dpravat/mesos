...
Future<short> poll(const int_fd& fd, short events);
 ...
 */
Future<std::string> read(const int_fd& fd);
 ...
 *     or made non-blocking.
 */
Future<Nothing> write(const int_fd& fd, const std::string& data);
 ...
 */
Future<Nothing> redirect(const int_fd& from, Option<int_fd> to,
                         size_t chunk = 4096);
 ...
 *     A failure will be returned if an error is detected.
 */
Future<std::string> peek(const int_fd& fd, size_t limit);
