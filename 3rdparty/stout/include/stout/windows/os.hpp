...
inline Try<Nothing> pipe(int pipe[2]) {
    ...
    pipe[0] = _open_osfhandle(reinterpret_cast<intptr_t>(read_handle), _O_RDONLY | _O_TEXT);
    if (pipe[0] == -1 ) {
      return ErrnoError();
    }
...
// The Windows handles received in the command line should be wrapped into
// file descriptors for use with Posix `read/write` functions.
