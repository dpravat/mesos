...
namespace os {
  ...
  public:
    ...
    void addReference(std::shared_ptr<Translator> ref) {
      adapter = ref;
    }
  ...
  public:
    ...
    Translator(SOCKETMODE dir) : direction(dir)
    {
      ...
      if (connect(PrivateSocket, (struct sockaddr *)&connect_addr, size) == SOCKET_ERROR)
      {
...
private:
  ...
  {
    std::shared_ptr<os::Translator> adapter = std::make_shared<os::Translator>(dir);
