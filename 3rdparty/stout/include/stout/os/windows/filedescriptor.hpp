...
public:
  ...
  WindowsFileDescriptor()
      : handle(INVALID_HANDLE_VALUE),
        socket(INVALID_SET_FILE_POINTER),
        crtFd(-1) {}

  WindowsFileDescriptor(HANDLE h)
      : handle(h), socket(INVALID_SOCKET), crtFd(-1) {
    crtFd = _open_osfhandle(reinterpret_cast<intptr_t>(handle), O_RDWR);
    CHECK_NE(crtFd, -1);
  }

  WindowsFileDescriptor(SOCKET s)
      : handle(INVALID_HANDLE_VALUE), socket(s), crtFd(-1) {
    CHECK(is_socket(socket));
  }

  WindowsFileDescriptor(int file)
      : handle(INVALID_HANDLE_VALUE), socket(INVALID_SOCKET), crtFd(file) {
  ...
  void addReference(std::shared_ptr<StdOut> ref) { adapter = ref; }
...
public:
  StdOut(SOCKETMODE dir) : direction(dir) {
    ...
    if (connect(conector, (struct sockaddr*)&connect_addr, size) ==
        SOCKET_ERROR) {
