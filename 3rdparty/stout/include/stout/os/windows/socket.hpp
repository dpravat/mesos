...
inline int bind(const FileDesc &socket, const struct sockaddr *addr,
                size_t addrlen) {
  CHECK_LE(addrlen, INT32_MAX);
  return ::bind(socket, addr, static_cast<int>(addrlen));
}

inline int connect(const FileDesc &socket, const struct sockaddr *address,
                   size_t addrlen) {
  CHECK_LE(addrlen, INT32_MAX);
  return ::connect(socket, address, static_cast<int>(addrlen));
}

inline ssize_t send(const FileDesc &sockfd, const void *buf, size_t len,
                    int flags) {
